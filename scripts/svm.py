...
def train_all(window_size, kernel = "poly"):
    svm_classifier(folder = "data/train_oil", window_size = window_size, kernel = kernel, save_folder = f"data/svm_models/{kernel}")
    svm_classifier(folder = "data/train_oil_vacuum", window_size = window_size, kernel = kernel, save_folder = f"data/svm_models/{kernel}")
    svm_classifier(folder = "data/train_oil_vacuum_teflon", window_size = window_size, kernel = kernel, save_folder = f"data/svm_models/{kernel}")
...
def eval_auto(kernel = "poly", test_set = "test_data_2"):
    result_df = eval_all(f"data/svm_models/{kernel}/results_0_5_{test_set}.csv", 0.5, kernel = kernel, test_set = test_set)

    res_2 = eval_all(f"data/svm_models/{kernel}/results_2_{test_set}.csv", 2, kernel = kernel, test_set = test_set)
    result_df = pd.concat([result_df, pd.DataFrame(res_2)])

    res_1 = eval_all(f"data/svm_models/{kernel}/results_1_{test_set}.csv", 1, kernel = kernel, test_set = test_set)
